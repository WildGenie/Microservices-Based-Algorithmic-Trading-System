    ...
    def stop(self):
        ...
        if self.strategy.p.backtest:   
            ...
            for i, d in enumerate(self.datas):
                data_size=len(d)
                ml_dict = {
                    "security": [d._name] * data_size,
                    "datetime": [
                        self.data.num2date(x)
                        for x in d.datetime.get(size=data_size)
                    ],
                }
                ...
                num_of_indicators = len(self.strategy.getindicators()) // len(
                    self.strategy.datas
                )
            ...
            Key = f'{str(self.strategy.db_run_id)}_ml_log.csv'
            csv_buffer = StringIO()
